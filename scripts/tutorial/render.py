...
for b in timeline["highlights"]:
    x = max(0, int(b["x"]) - 3)
    y = max(0, int(b["y"]) - 3)
    w = min(1600 - x, int(b["width"]) + 6)
    h = min(800 - y, int(b["height"]) + 6)
    if w > 0 and h > 0:
        filters.append(
            f"drawbox=x={x}:y={y}:w={w}:h={h}:color=0x26b6a5@0.9:t=3:enable='between(t,{b['start']:.3f},{b['end']:.3f})'"
        )
...
subprocess.run(
    [
        ...
        "+faststart",
        "-an",
        str(out / "MacroTrading-Dashboard-Tutorial.mp4"),
    ],
    check=True,
)
subprocess.run(
