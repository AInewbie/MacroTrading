...
def render_html(result: dict[str, Any], generated_at: str, title: str = "MacroTrading Five-Theme Review") -> str:
    ...
    for theme_id, theme in themes.items():
        ...
        component_strip = ""
        if components:
            component_strip = '<div class="components">' + "".join(
                f'<span><b>{escape(key)}</b>{float(components[key]):g}</span>'
                for key in ("P", "F", "M", "C", "X", "R")
            ) + "</div>"
        market = theme["market"]
        if market["m_component"] is None:
            market_text = "Market component not refreshed: no eligible completed-session observation was supplied."
        else:
            relative = market["relative_from_anchor_pct"]
            relative_text = "unavailable" if relative is None else f"{relative:+.2f}% from anchor"
            breach = market["breach_side"] or "none"
            market_text = f'M={market["m_component"]:g} · {relative_text} · confirmed breach: {breach}'
        cards.append(f"""
        <article class="theme-card" id="{escape(theme_id)}">
          ...
          <p class="market"><b>Market confirmation</b> {escape(market_text)}</p>
...
.components{{display:flex;flex-wrap:wrap;gap:7px;margin:-6px 0 14px}} .components span{{display:flex;gap:8px;background:#eef2ff;border:1px solid #dbe3ff;border-radius:999px;padding:4px 9px;font-size:12px}} .components b{{color:var(--blue)}} .market{{font-size:13px;color:#475569;border-left:3px solid #93c5fd;padding:8px 11px;background:#f8fbff;border-radius:0 9px 9px 0}}
