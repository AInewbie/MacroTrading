...
class Store:
    ...
    def get(self, key, default=None, db=None):
        ...
        row = db.execute(
            "SELECT version,payload FROM objects WHERE key=?", (key,)
        ).fetchone()
    ...
    def put(self, db, key, value, expected=None):
        ...
        if expected is not None and version != expected:
            raise Conflict(
                "Workspace changed in another window. Reload before retrying."
            )
        db.execute(
            "INSERT INTO objects(key,version,payload) VALUES(?,?,?) ON CONFLICT(key) DO UPDATE SET version=excluded.version,payload=excluded.payload",
            (key, version + 1, canonical(value)),
        )
        return version + 1
    ...
    def audit(self, db, action, payload):
        ...
        prior = previous["hash"] if previous else ""
        at = stamp()
        item = {"at": at, "action": action, "payload": payload, "previous_hash": prior}
        h = digest(item)
        db.execute(
            "INSERT INTO audit(at,action,payload,previous_hash,hash) VALUES(?,?,?,?,?)",
            (at, action, canonical(payload), prior, h),
        )
        return h
    ...
    def verify_audit(self):
        ...
        with closing(self.connect()) as db:
            for r in db.execute("SELECT * FROM audit ORDER BY seq"):
                ...
                if r["previous_hash"] != previous or digest(item) != r["hash"]:
                    return {"valid": False, "row": r["seq"]}
                previous = r["hash"]
                count += 1
    ...
    def runs(self):
        with closing(self.connect()) as db:
            return [
                {**dict(r), "manifest": json.loads(r["manifest"])}
                for r in db.execute(
                    "SELECT id,as_of,created_at,manifest FROM runs ORDER BY created_at DESC LIMIT 200"
                )
            ]
    ...
    def run(self, id):
        with closing(self.connect()) as db:
            ...
            return {
                **dict(r),
                **{k: json.loads(r[k]) for k in ("manifest", "input", "result")},
            }
    ...
    def inbox(self):
        with closing(self.connect()) as db:
            return [
                {**dict(r), "payload": json.loads(r["payload"])}
                for r in db.execute(
                    "SELECT * FROM inbox ORDER BY first_known_at DESC LIMIT 1000"
                )
            ]
    ...
    def source_health(self):
        with closing(self.connect()) as db:
            return [
                dict(r)
                for r in db.execute(
                    "SELECT id,source_id,at,status,content_hash,error,items FROM source_fetches WHERE id IN (SELECT MAX(id) FROM source_fetches GROUP BY source_id)"
                )
            ]
    ...
    def decisions(self):
        with closing(self.connect()) as db:
            return [
                {**dict(r), "payload": json.loads(r["payload"])}
                for r in db.execute(
                    "SELECT * FROM decisions ORDER BY at DESC LIMIT 500"
                )
            ]
